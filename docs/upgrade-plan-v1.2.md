...
### B1. ❌ 一次性 / 临时收支记录 —— 不做（产品定位决定）
~~新增 transactions 表逐笔记账~~
- **决定（2026-06）**：明确**不做**逐笔小额流水。本应用定位为「现金流追踪器」而非「记账应用」：
  用户通过**周期性项目（收入/投资/账单/订阅）+ 手动更新现金余额**来反映财务状态，
  不追求记录每一笔咖啡/打车。这是有意的极简定位，区别于 Money Forward / YNAB。
- **连带影响**：B2（分类统计）、B4（信用卡逐笔）依赖逐笔流水，**一并不做**。
...
### B4. ❌ 信用卡逐笔消费 —— 不做（依赖 B1）
保持现状：每月手填账单总额 `statement_amount`，符合「手动维护数字」的定位。
